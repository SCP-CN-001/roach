...
from roach.utils.common import load_entry_point
...
class PpoPolicy(nn.Module):
    def __init__(
        ...
        features_extractor_entry_point = features_extractor_entry_point.replace(
            "agents.rl_birdview", "roach"
        )
        ...
        distribution_entry_point = distribution_entry_point.replace(
            "agents.rl_birdview", "roach"
        )
    ...
    def forward(
        ...
        with th.no_grad():
            if feature_input is None:
                obs_tensor_dict = {
                    k: th.as_tensor(v).to(self.device).unsqueeze(0)
    ...
    def load(cls, path):
        ...
        for index in [2, 4, 6, 8, 10]:
            state_dict[
                "features_extractor.cnn." + str(index // 2) + ".weight"
            ] = state_dict.pop("features_extractor.cnn." + str(index) + ".weight")
            state_dict[
                "features_extractor.cnn." + str(index // 2) + ".bias"
            ] = state_dict.pop("features_extractor.cnn." + str(index) + ".bias")
